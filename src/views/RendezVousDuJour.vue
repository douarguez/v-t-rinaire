...
function formatHeure(datetime) {
  return new Date(datetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
}

const appointments = computed(() => {
  const today = new Date().toDateString()
  return interventionStore.evenements
    .filter(i => new Date(i.start).toDateString() === today)
    .map(i => {
      const a = animalStore.getById(i.animalId)
      const c = a ? clientStore.getById(a.clientId) : null
      return {
        time: formatHeure(i.start),
        petName: a?.nom || '—',
        owner: c ? `${c.nom} ${c.prenom}` : '—'
      }
    })
})
</script>
